	...
	function add_friend_from_search(compid, elementname){
		xhr = $.ajax({
		        url: 'add-friend',
		        type: 'POST', 
		        data: {
		...
		xhr.done( () =>{
			if ($("#" + compid + "-request").length > 0) {
				...
				xhr3.done( () => {
					populate_requestlist(JSON.parse(xhr3['responseText']));
				});
			} else{
				console.log("here");
				xhr3 = $.ajax({
	...
	function populate_requestlist(requests){
	...
	function populate_requested(requests){
